...
namespace Pinwheel.Griffin.Rendering
{
    ...
    public class GTreeRenderer3
    {
        ...
        internal GTreeRenderer3(GStylizedTerrain terrain, int quadTreeDepth)
        {
        ...
        internal void OnPrototypesChanged()
        {
            OnDisable();
            OnEnable();
        }
        ...
        private void DrawInstances(Camera cam)
        {
            for (int i = 0; i < perPrototypeInstanceData.Count; ++i)
            {
                ...
                if (billboard != null)
                {
                    if (renderData.UpdateBillboardInfo()) //hack to get around null billboard mesh at startup
                    {
        ...
        class PrototypeRenderData
        {
            ...
            private LodInfo GetBillboardInfo(GTreePrototype p)
            {
                ...
                {
                    Debug.LogWarning($"Missing billboard material: {p.Billboard.name}", p.Billboard);
                    return null;
                }
            ...
            public bool UpdateBillboardInfo()
            {
                if (billboard != null)
                {
                    ...
                    if (bbMat == null)
                    {
                        Debug.LogWarning($"Missing billboard material {billboardAsset.name}", billboardAsset);
                        return false;
                    }
