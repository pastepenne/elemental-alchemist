Shader "Hidden/Griffin/TextureStamperBrush"
{
		...
		SubShader
	{
		...
		Pass
		{
			...
			#pragma shader_feature BLEND_SPLAT_WEIGHT
			...
			sampler2D _SplatControlMap;
			float _SplatControlChannel;
			sampler2D _SplatControlRemap;
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				float blendSplatWeightValue = 1;
				#if BLEND_SPLAT_WEIGHT
					float4 weightMapValue = tex2D(_SplatControlMap, i.normalizePos);
					blendSplatWeightValue = tex2D(_SplatControlRemap, float2(weightMapValue[uint(_SplatControlChannel)], 0.5));
				#endif
				...
				float result = blendHeightValue * blendSlopeValue * blendNoiseValue * blendSplatWeightValue * maskValue * falloffValue * insideStamperValue;
