...
namespace Pinwheel.Griffin.PaintTool
{
    ...
    public class GTerrainPainterInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            GEditorCommon.DrawLivePreviewToggle(ref GEditorSettings.Instance.paintTools.enableLivePreview);
        ...
        private class GBrushGUI
        {
            ...
            public static readonly GUIContent METALLIC = new GUIContent("Metallic", "Value written to the Metallic map R channel");
            public static readonly GUIContent SMOOTHNESS = new GUIContent("Smoothness", "Value written to the Metallic map A channel");
        ...
        private void DrawBrushGUI()
        {
            GEditorCommon.Foldout(GBrushGUI.LABEL, true, GBrushGUI.ID, () =>
            {
                ...
                if (painter.Mode == GTexturePaintingMode.Metallic)
                {
                    Color color = painter.BrushColor;
                    color.r = EditorGUILayout.Slider(GBrushGUI.METALLIC, color.r, 0f, 1f);
                    painter.BrushColor = color;
                }
                else if (painter.Mode == GTexturePaintingMode.Smoothness)
                {
                    Color color = painter.BrushColor;
                    color.a = EditorGUILayout.Slider(GBrushGUI.SMOOTHNESS, color.a, 0f, 1f);
                    painter.BrushColor = color;
                }
                else
                {
        ...
        private class GRulesGUI
        {
            ...
            public static readonly GUIContent PRO_FEATURE_LABEL = new GUIContent("Conditional painting", GEditorSkin.Instance.GetTexture("IconWhite"));
            public static readonly GUIContent PRO_FEATURE_DECS = new GUIContent("Paint on terrains with height, slope and noise rule. Available in Highland and Summit edition.");
        ...
        private void DrawRulesGUI()
        {
            GEditorCommon.Foldout(GRulesGUI.LABEL, false, GRulesGUI.ID, () =>
            {
                ...
                {
                    EditorGUILayout.LabelField(GRulesGUI.PRO_FEATURE_LABEL);
                    EditorGUILayout.LabelField(GRulesGUI.PRO_FEATURE_DECS, GStyles.P1);

                    configs.BlendHeight = false;
                    configs.BlendSlope = false;
                    configs.BlendNoise = false;
                }

                GUI.enabled = GRuntimeSettings.isHighlandEditionInstalled;
...
#if UNITY_EDITOR_OSX
            GEditorSettings.Instance.paintTools.useSimpleCursor = true;
#endif
