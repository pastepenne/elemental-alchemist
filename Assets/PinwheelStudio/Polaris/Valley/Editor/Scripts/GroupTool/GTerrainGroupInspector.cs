    ...
    public class GTerrainGroupInspector : Editor
    {
        ...
        private void DrawGeometryOverrideGUI()
        {
            ...
            GEditorCommon.Foldout(label, false, id, () =>
            {
                ...
                {
                    g.OverrideLodCount = false;
                    g.LODCount = 1;
                    GUI.enabled = false;
                    GUIContent label = EditorGUIUtility.TrTextContent("LOD Count", "Additional gear in Highland or Summit edition", GEditorSkin.Instance.GetTexture("IconWhite"));
                    EditorGUILayout.Toggle(false, GUILayout.Width(TOGGLE_WIDTH));
                    EditorGUILayout.DelayedIntField(label, 1);
                    GUI.enabled = true;
                }
                EditorGUILayout.EndHorizontal();
        ...
        private void DrawFoliageOverrideGUI()
        {
            ...
            GEditorCommon.Foldout(label, false, id, () =>
            {
                ...
                {
                    f.OverrideEnableInteractiveGrass = false;
                    f.EnableInteractiveGrass = false;
                    GUI.enabled = false;
                    GUIContent label = EditorGUIUtility.TrTextContent("Interactive Grass", "Additional feature in Highland or Summit edition", GEditorSkin.Instance.GetTexture("IconWhite"));
                    EditorGUILayout.Toggle(false, GUILayout.Width(TOGGLE_WIDTH));
