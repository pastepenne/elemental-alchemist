...
namespace Pinwheel.Griffin
{
    ...
    public class GStylizedTerrainInspector : Editor
    {
        ...
        private class GGeometryGUI
        {
            ...
            public static readonly GUIContent CONTEXT_MATCH_HEIGHT_MAP_TO_VERTICES = new GUIContent("Advanced/Match Height Map pixels to vertices");
            ...
            public static readonly GUIContent LOD_COUNT_LOCKED = new GUIContent("LOD Count", GEditorSkin.Instance.GetTexture("IconWhite"), "Number of LOD for each chunk.\nAdditional gear in Highland or Summit edition.");
        ...
        private void DrawGeometryGUI()
        {
            ...
            menu.AddSeparator(null);
            menu.AddItem(
                new GUIContent(GGeometryGUI.CONTEXT_MATCH_HEIGHT_MAP_TO_VERTICES),
                false,
                () => { BeginMatchHeightMapPixelsToVertices(); });
            menu.AddItem(
...
#if !GRIFFIN_BURST
            if (GRuntimeSettings.isSummitEditionInstalled)
            {
            ...
            GEditorCommon.Foldout(GGeometryGUI.LABEL, false, GGeometryGUI.ID, () =>
            {
                ...
                else
                {
                    settings.LODCount = 1;
                    GUI.enabled = false;
                    EditorGUILayout.DelayedIntField(GGeometryGUI.LOD_COUNT_LOCKED, 1);
                    GUI.enabled = true;
                }
        ...
        private void BeginMatchHeightMapPixelsToVertices()
        {
            int meshResolution = terrain.TerrainData.Geometry.MeshResolution;
            int chunkGridSize = terrain.TerrainData.Geometry.ChunkGridSize;

            int pixelCountTotal = GGeometry.GetPixelCountByMeshResolution(meshResolution, chunkGridSize);
            int heightMapResolution = terrain.TerrainData.Geometry.HeightMapResolution;

            if (pixelCountTotal == heightMapResolution)
            {
                EditorUtility.DisplayDialog(
                    "Match Height Map pixels to vertices",
                    $"Already matched with Height Map Resolution at {heightMapResolution}",
                    "OK");
            }
            else
            {
                if (EditorUtility.DisplayDialog(
                    "Match Height Map pixels to vertices",
                    $"You're about to change Height Map Resolution from {heightMapResolution} to {pixelCountTotal}. This is a destructive operation, proceed?",
                    "Change Height Map Resolution", "Don't change"))
                {
                    bool deferredUpdate = EditorPrefs.GetBool(GEditorCommon.GetProjectRelatedEditorPrefsKey(GGeometryGUI.DEFERRED_UPDATE_KEY), false);
                    terrain.TerrainData.Geometry.HeightMapResolution = pixelCountTotal;
                    if (EditorGUI.EndChangeCheck() && !deferredUpdate)
                    {
                        data.Geometry.SetRegionDirty(new Rect(0, 0, 1, 1));
                        data.SetDirty(GTerrainData.DirtyFlags.GeometryTimeSliced);
            ...
            GEditorCommon.Foldout(GShadingGUI.LABEL, false, GShadingGUI.ID, () =>
            {
                ...
                if (GCommon.CurrentRenderPipeline == GRenderPipelineType.Builtin)
                {
                    GEditorCommon.DrawAffLinks(
                        "Edit terrain shaders in BiRP with Amplify Shader Editor",
                        "https://assetstore.unity.com/packages/tools/visual-scripting/amplify-shader-editor-68570",
                        "", GAssetLink.UTM_SOURCE, "terrain-inspector-ase", GUILocation.InspectorTerrain);
                }
#endif
#if !__MICROSPLAT_POLARIS__
                GEditorCommon.DrawAffLinks(
                    ...
                    "", GAssetLink.UTM_SOURCE, "terrain-inspector-MS", GUILocation.InspectorTerrain);
#endif
            }, menu);
        }
        ...
        private class GFoliageGUI
        {
            ...
            public static readonly GUIContent INTERACTIVE_GRASS_LOCKED = new GUIContent("Interactive Grass", GEditorSkin.Instance.GetTexture("IconWhite"), "Toggle grass bending when the player passes by. Additional feature in Highland or Summit edition");
        ...
        private void DrawFoliageGUI()
        {
            ...
            GEditorCommon.Foldout(GFoliageGUI.LABEL, false, GFoliageGUI.ID, () =>
            {
                ...
                {
                    settings.EnableInteractiveGrass = false;
                    GUI.enabled = false;
                    EditorGUILayout.Toggle(GFoliageGUI.INTERACTIVE_GRASS_LOCKED, false);
                    GUI.enabled = true;
                }
                ...
                GEditorCommon.DrawAffLinks(
                    ...
                    "", GAssetLink.UTM_SOURCE, "terrain-inspector-vegetation", GUILocation.InspectorTerrain);
            },
            menu,
            headerWarning);
        }
        ...
        private class GMaskGUI
        {
            ...
            public static readonly string R_TEXT = "Lock regions from being edited.";
            ...
            if (!GPackageInitializer.isVistaInstalled)
            {
                GEditorCommon.Foldout(GProcTerrainGUI.LABEL, true, GProcTerrainGUI.ID, () =>
                {
                    ...
                    GEditorCommon.DrawAffLinks(
                        ...
                        "", GAssetLink.UTM_SOURCE, "terrain-inspector-proc-terrain", GUILocation.InspectorTerrain);
                });
            }
        }
        ...
        private GStylizedTerrain CreateNeighborTerrain()
        {
            ...
            //Default group id is 0, copy the id from source terrain so they can be connected.
            t.GroupId = terrain.GroupId;

            //After cloning from source material, the new material can refer to splat texture or color texture of source terrain, causing incorrect visual.
            //Mark its shading as dirty to assign correct texture references.
