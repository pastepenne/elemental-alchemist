...
namespace Pinwheel.Griffin
{
    ...
    {
        private class GAssetPreviewCacheEntry
        {
            public Texture2D texture;
            public bool isFullPreview;
        }

        private static Dictionary<int, GAssetPreviewCacheEntry> assetPreviewCache = new Dictionary<int, GAssetPreviewCacheEntry>();

        static GEditorCommon()
        {
            AssemblyReloadEvents.beforeAssemblyReload += ReleaseAssetPreviewCache; 
        }
        ...
        public static void DrawTreePreviewSingle(Rect r, object o)
        {
            ...
            if (t != null)
            {
                Texture2D preview = GetAssetPreviewCached(t.Prefab);
                if (preview != null)
                {
        ...
        public static void DrawGameObjectPreview(Rect r, object o)
        {
            ...
            if (g != null)
            {
                Texture2D preview = GetAssetPreviewCached(g);
                if (preview != null)
                {
                    EditorGUI.DrawPreviewTexture(r, preview, null, ScaleMode.ScaleToFit);
                }
        ...
        public static void DrawPreview(Rect r, Object o, Color tint)
        {
            ...
            Texture2D preview = GetAssetPreviewCached(o);
            if (preview != null)
            {
        ...
        public static Texture2D GetAssetPreviewCached(Object o)
        {
            if (o == null)
            {
                return null;
            ...
            GAssetPreviewCacheEntry cachedEntry;
            if (assetPreviewCache.TryGetValue(instanceId, out cachedEntry))
            {
                if (cachedEntry != null && cachedEntry.texture != null && cachedEntry.isFullPreview)
                {
            ...
            if (preview != null)
            {
                return CacheAssetPreview(instanceId, preview, true);
            }
            ...
            Texture2D miniThumbnail = AssetPreview.GetMiniThumbnail(o);
            if (miniThumbnail != null)
            {
                return CacheAssetPreview(instanceId, miniThumbnail, false);
            }

            return null;
        }

        private static Texture2D CacheAssetPreview(int instanceId, Texture2D source, bool isFullPreview)
        {
            if (source == null)
            {
                return null;
            ...
            GAssetPreviewCacheEntry entry;
            if (assetPreviewCache.TryGetValue(instanceId, out entry))
            {
                if (entry != null &&
                    entry.texture != null &&
                    entry.isFullPreview == isFullPreview)
                {
            ...
            Texture2D textureCopy = GCommon.CloneTexture(source);
            textureCopy.hideFlags = HideFlags.HideAndDontSave;

            assetPreviewCache[instanceId] = new GAssetPreviewCacheEntry()
            {
                texture = textureCopy,
        ...
        private static void ReleaseAssetPreviewCache()
        {
            foreach (GAssetPreviewCacheEntry entry in assetPreviewCache.Values)
            {
                if (entry != null && entry.texture != null)
                {
                    Object.DestroyImmediate(entry.texture);
