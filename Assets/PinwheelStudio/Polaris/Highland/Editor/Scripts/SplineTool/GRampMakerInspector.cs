...
namespace Pinwheel.Griffin.SplineTool
{
    ...
    public class GRampMakerInspector : Editor
    {
        ...
        {
            List<GOverlapTestResult> sweepTests = GetSweepTests();
        ...
        {
            List<GStylizedTerrain> terrains = GUtilities.ExtractTerrainsFromOverlapTest(GetSweepTests());
            for (int i = 0; i < terrains.Count; ++i)
            {
                UpdatePreview(terrains[i]);
            }
        }

        private void UpdatePreview(GStylizedTerrain terrain)
        {
            if (terrain == null)
                return;
            if (terrain.transform.rotation != Quaternion.identity ||
                terrain.transform.lossyScale != Vector3.one)
                return;

            RenderTexture rt = GetPreviewTexture(terrain);
            instance.Internal_DrawOnTexture(terrain, rt);
        }

        private List<GOverlapTestResult> GetSweepTests()
        {
            if (instance == null || instance.SplineCreator == null)
                return new List<GOverlapTestResult>();
            return instance.SplineCreator.SweepTest(instance.Curviness, instance.Width, instance.FalloffWidth);
        }
