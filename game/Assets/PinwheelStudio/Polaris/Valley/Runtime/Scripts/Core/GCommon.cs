...
namespace Pinwheel.Griffin
{
    ...
    public static class GCommon
    {
        ...
        public const string ONLINE_MANUAL = "https://docs.pinwheelstud.io/polaris/3/";
        ...
        internal delegate void GetMaxLodCountHandler(ref int lodCount);
        internal static event GetMaxLodCountHandler getMaxLodCountCallback;
        public static int MAX_LOD_COUNT
        {
            get
            {
                int maxLodCount = 1;
                if (getMaxLodCountCallback != null)
                {
                    getMaxLodCountCallback.Invoke(ref maxLodCount);
                }
        ...
        public static void DrawTexture(RenderTexture rt, Texture texture, Rect uvRect, Material mat, int pass = 0)
        {
            ...
            mat.SetVector("_MainTex_TexelSize", new Vector4(texture.texelSize.x, texture.texelSize.y, texture.width, texture.height));
            mat.SetPass(pass);
            GL.LoadOrtho();
            GL.Begin(GL.QUADS);
        ...
        public static GTerrainGeneratedData GetTerrainGeneratedDataAsset(GTerrainData terrainData, string namePrefix = "Generated")
        {
            ...
            if (!Application.isPlaying)
            {
                ...
                string filePath = Path.Combine(directory, string.Format("{0}_{1}.asset", terrainData.Id, namePrefix));
